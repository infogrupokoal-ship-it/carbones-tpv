...
def _preparar_catalogo(db: Session):
    productos = db.query(Producto).filter(Producto.is_active).all()
    catalogo_text = "\n".join(
        [
            f"- ID: {p.id} | Nombre: {p.nombre} | Precio: {p.precio}€ | Stock: {p.stock_actual}"
            for p in productos
        ]
    )
...
def procesar_mensaje_whatsapp(
    db: Session, sender_phone: str, mensaje: str, responsables: list
):
    ...
    def tool_actualizar_stock_cocina(
        producto_id: int,
        ...
        return ai_tools.actualizar_stock_cocina(
            db, producto_id, cantidad_anadida, precio_nuevo, alergenos
        )
    ...
    try:
        model = genai.GenerativeModel(
            model_name, system_instruction=system_instruction, tools=tools_to_use
        )
