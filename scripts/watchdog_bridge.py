...
def start_process(name, path):
    logging.info(f"ðŸš€ Iniciando componente: {name} ({os.path.basename(path)})...")
    try:
        ...
        logging.error(f"âŒ Error al iniciar {name}: {e}")
        return None

def monitor():
    logging.info("ðŸ• Master Watchdog Industrializado activo.")
    ...
    while True:
        for name, path in SCRIPTS.items():
            ...
            if proc is None or proc.poll() is not None:
                if proc is not None:
                    logging.warning(f"âš ï¸ Componente {name} se detuvo inesperadamente (Exit Code: {proc.returncode}).")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.info("ðŸ›‘ Master Watchdog detenido manualmente.")
        sys.exit(0)
