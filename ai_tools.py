...
def registrar_pedido_kiosko(db, cliente_id: int, items: list, origen: str = "WHATSAPP"):
    ...
    """
    from models import Producto, Pedido, ItemPedido, MovimientoStock
        ...
        if prod:
            ...
            if prod.stock_base_id:
                ...
                if parent:
                    ...
                    db.add(MovimientoStock(producto_id=parent.id, cantidad=-(cantidad * prod.factor_stock), tipo="VENTA", origen_id=nuevo_pedido.id, descripcion=f"Venta IA (Ref: {prod.nombre})"))
            else:
...
def actualizar_stock_cocina(db, producto_id: int, cantidad_anadida: int, precio_nuevo: float = None, alergenos: str = None):
    """
    ...
    db.commit()
    return {"status": "ok", "producto": prod.nombre, "stock_actualizado": prod.stock_actual}
