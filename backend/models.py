...
class Producto(Base):
    ...
    stock_base_id = Column(
        String(36), ForeignKey("productos.id"), nullable=True
    )  # Si es un hijo (Ej. Cuarto Pollo) apunta a Pollo General
    factor_stock = Column(
        Float, default=1.0
    )  # Cuanto resta al padre por unidad vendida
...
class Cliente(Base):
    ...
    estado_registro = Column(
        String, default="COMPLETADO"
    )  # PENDIENTE_NOMBRE, COMPLETADO
...
class Pedido(Base):
    ...
    )  # ESPERANDO_PAGO -> EN_PREPARACION -> LISTO
...
class MovimientoStock(Base):
    ...
    )  # ID del pedido si es venta, o nulo si es produccion manual
    descripcion = Column(String, nullable=True)
    is_synced = Column(Boolean, default=False)
    remote_id = Column(String(36), nullable=True)
...
class LogOperativo(Base):
    """Registro técnico para auditoría de procesos en segundo plano."""
    __tablename__ = "logs_operativos"
    id = Column(
        String(36), primary_key=True, default=lambda: str(uuid.uuid4()), index=True
    )
    modulo = Column(String, index=True)  # SCHEDULER, SYNC, AI, DATABASE
    nivel = Column(String)  # INFO, WARNING, ERROR, CRITICAL
    mensaje = Column(String)
    detalles = Column(String, nullable=True)  # JSON o Traceback
    fecha = Column(DateTime, default=datetime.now)
