...
def seed():
    ...
    try:
        ...
        if not cat1:
            ...
            # --- PRODUCTOS NORMALES ---
            prod3 = Producto(nombre="Patatas Fritas", precio=3.00, categoria_id=cat2.id, stock_actual=50)
            prod4 = Producto(nombre="Croquetas", precio=1.00, categoria_id=cat2.id, stock_actual=100)
            prod5 = Producto(nombre="Refresco Cola", precio=1.50, categoria_id=cat3.id, stock_actual=200)
            db.add_all([prod3, prod4, prod5])
            
            # --- LOGICA FRACCIONAL DEL POLLO ---
            # Pollo invisible base (Unidad minima = 1 Cuarto)
            pollo_base = Producto(nombre="[MATERIA] Pollo Base", precio=0, categoria_id=None, is_active=False, stock_actual=100) # 100 cuartos = 25 pollos enteros
            db.add(pollo_base)
            db.flush()
            
            # Sub-productos (Los factors son cuartos: 4 cuartos = 1 entero, 2 cuartos = 1 medio, 1 cuarto = cuarto)
            p_entero = Producto(nombre="Pollo Entero", precio=10.00, categoria_id=cat1.id, stock_base_id=pollo_base.id, factor_stock=4)
            p_medio = Producto(nombre="Medio Pollo", precio=5.50, categoria_id=cat1.id, stock_base_id=pollo_base.id, factor_stock=2)
            p_cuarto = Producto(nombre="Cuarto de Pollo", precio=3.00, categoria_id=cat1.id, stock_base_id=pollo_base.id, factor_stock=1)
